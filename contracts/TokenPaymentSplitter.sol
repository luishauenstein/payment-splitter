...
abstract contract TokenPaymentSplitter {
  ...
  function _addPayee(address account, uint256 shares_) internal {
    require(account != address(0), 'TokenPaymentSplitter: account is the zero address');
    require(shares_ > 0, 'TokenPaymentSplitter: shares are 0');
    require(_shares[account] == 0, 'TokenPaymentSplitter: account already has shares');
  ...
  function release(address account) public virtual {
    require(_shares[account] > 0, 'TokenPaymentSplitter: account has no shares');

    uint256 tokenTotalReceived =
      IERC20(paymentToken).balanceOf(address(this)) + _totalTokenReleased;
    uint256 payment =
      (tokenTotalReceived * _shares[account]) / _totalShares - _tokenReleased[account];
