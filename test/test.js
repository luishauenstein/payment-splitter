...
describe('TokenPaymentSplitter Tests', () => {
  ...
  describe('Add payees with varying amounts and distribute payments', async () => {
    ...
    it('payment token is distributed evenly to multiple payees', async () => {
      ...
      await mockPool.connect(account1).release(account1.address);
      await mockPool.connect(account2).release(account2.address);

      await mockPool.connect(account3).release(account3.address);

      await mockPool.connect(account4).release(account4.address);
    ...
    it('payment token is distributed unevenly to multiple payees', async () => {
      ...
      const mockPoolTestPaymentTokenBalance = await testPaymentToken.balanceOf(mockPool.address);
